...
import FolderIcon from "../../../../../assets/Employee_asserts/Family/Folder.svg";
import styles from "./FamilyInfoAccordion.module.css"; // reuse your CSS module layout
import FamilyInfoCard from "../../../../../widgets/Employee_widgets/InfoCard/FamilyInfoCard";
...
const FamilyInfoAccordion = ({ expanded, onChange, familyMembers = [], isLoading, isError }) => {
  // Transform API data to the format expected by FamilyInfoCard
  const transformFamilyData = (member) => {
    if (!member) return [];

    return [
      { label: "Name", value: member.fullName || "N/A" },
      { label: "Blood Group", value: member.bloodGroup || "N/A" },
      { label: "Nationality", value: member.nationality || "N/A" },
      { label: "Occupation", value: member.occupation || "N/A" },
      { label: "Email Id", value: member.email || "N/A" },
      { label: "Phone Number", value: member.contactNumber ? `+91 ${member.contactNumber}` : "N/A" },
      { label: "Aadhaar No", value: member.adhaarNo || "N/A" },
      { label: "Gender", value: member.gender || "N/A" },
    ];
  };

  // Find father and mother from family members
  const father = familyMembers?.find(member => member.relation?.toLowerCase() === "father");
  const mother = familyMembers?.find(member => member.relation?.toLowerCase() === "mother");

  const fatherData = transformFamilyData(father);
  const motherData = transformFamilyData(mother);
