...
// import { basicInfoSchema } from "../../../../../utils/Employee_utils/OnboardingSchemas";
// import axios from "axios";
// import FormValidationAlert from "../../../../../utils/Employee_utils/FormValidationAlert";
...
// } from "../../../../../queries/Employee_queries/onBoardingForms/dropDownApi/useEmployeeFormData";
...
const EmployeeOnboardingForm = forwardRef(
  ({ onTempIdGenerated, tempId, isEditMode }, ref) => {
    const { user } = useAuth();
    const location = useLocation();
    const hrEmployeeId = user?.employeeId || 5109;
   ...
   const getEmpTypeIdFromState = useCallback(() => {
  const typeStr = location.state?.employeeType;
  if (typeStr === "Teach") return 1;
  if (typeStr === "Non Teach") return 2;
  return "";
}, [location.state]);
      ...
      if (tempId) {
          ...
          try {
            ...
            if (response.data && formikRef.current) {
              ...
              formikRef.current.setValues({
                ...
                email: data.email || data.personalEmail || "",
                emergencyPhNo:
                  data.emergencyPhoneNo || data.emergencyPhNo || "",
                empWorkModeId:
                  Number(data.workingModeId) ||
                  Number(data.empWorkModeId) ||
                  "",
                joinTypeId:
                  Number(data.joiningAsTypeId) ||
                  Number(data.joinTypeId) ||
                  "",
                dateOfJoin: formatDate(
                  data.dateOfJoining || data.dateOfJoin || data.doj
                ),
                pancardNum: data.pancardNo || data.pancardNum || "",
                previousChaitanyaId:
                  data.preChaitanyaId || data.previousChaitanyaId || "",
                referenceEmployeeId:
                  Number(data.referenceEmpId) ||
                  Number(data.referenceEmployeeId) ||
                  "",
                empTypeId: data.empTypeId || getEmpTypeIdFromState(),

                emergencyRelationId:
                  Number(data.emergencyRelationId) ||
                  Number(data.relationId) ||
                  "",
                dateOfBirth: formatDate(data.dateOfBirth || data.dob),
                contractStartDate: formatDate(data.contractStartDate),
                contractEndDate: formatDate(data.contractEndDate),
                ...
                managerId: Number(data.managerId) || "",
                hiredByEmpId:
                  Number(data.hiredByEmpId) ||
                  Number(data.hiredByEmp) ||
                  "",
                reportingManagerId:
                  Number(data.reportingManagerId) ||
                  Number(data.reportingManager) ||
                  "",
                replacedByEmpId: Number(data.replacedByEmpId) || "",
              });
            }
        ...
        if (formikRef.current) {
          formikRef.current.setFieldValue(
            "empTypeId",
            getEmpTypeIdFromState()
          );
        }
      }
    }, [tempId, location.state, getEmpTypeIdFromState]);
        ...
        onSubmit={async (values, { setSubmitting, setFieldValue }) => {
          try {
            ...
            const finalPayload = {
              ...
              fatherName: toTitleCase(values.fatherName),
              adhaarName: toTitleCase(values.adhaarName),
