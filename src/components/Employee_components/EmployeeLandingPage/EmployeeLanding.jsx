...
import EmployeeCardWithoutCheckBox from "../../../widgets/Employee_widgets/EmployeeDetailsCard/EmployeeCardWithoutCheckBox";
...
import { searchEmployee } from "../../../queries/Employee_queries/EmployeeSearch/EmployeeSearch";
...
const EmployeeLanding = () => {
  ...
  const [empIdResult, setEmpIdResult] = useState(null);
  const [filterResults, setFilterResults] = useState([]);
  const [hasSearched, setHasSearched] = useState(false);

  // ---------------------------
  // STATIC RECENT SEARCH (6 CARDS)
  // ---------------------------
  const recentEmployees = Array.from({ length: 6 }).map(() => ({
    id: "HYD5627182",
    name: "Devansh.N",
    dept: "IT Cell",
    image: empprofile,
 ...
 const handleSearchSubmit = async () => {
  ...
  // FILTER SEARCH
  // ---------------------------
  const handleFilterSearch = async ({ cityId, employeeTypeId, campusId }) => {
    if (!cityId || !employeeTypeId || !campusId) {
      alert("City, Employee Type, and Campus are mandatory");
      return;
    }
  ...
  // NAVIGATION
  // ---------------------------
  const navigateToProfile = (emp) => {
    const routingId =
      emp.payRollId || emp.tempPayrollId || emp.empId;

    navigate(
      `/scopes/employee/profile-overview/${routingId}/basic-info`
    );
  };
  ...
  return (
    <div className={styles.wrapper_container}>
          ...
          <div className={styles.employeeGrid}>
            {recentEmployees.map((emp, index) => (
              <div
                ...
                onClick={() =>
                  navigate(
                    `/scopes/employee/profile-overview/${emp.id}/basic-info`
                  )
                }
              >
                <EmployeeCardWithoutCheckBox employee={emp} />
              </div>
            ))}
          </div>
        </>
      ...
      {hasSearched && (
        <>
          <h4 className={styles.recentSearchTitle}>Employee Results</h4>
          ...
          <div className={styles.employeeGrid}>
            ...
            {empIdResult && (
              <div
                style={{ cursor: "pointer" }}
                onClick={() => navigateToProfile(empIdResult)}
              >
                <EmployeeCardWithoutCheckBox
                  employee={{
                    id: empIdResult.payRollId,
                    name: empIdResult.empName,
                    dept: empIdResult.departmentName,
                    image: empprofile,
                    styleImg: blueLine,
                    level: empIdResult.employeeTypeName,
                    status: empIdResult.modeOfHiringName,
                  }}
                />
              </div>
            )}
            ...
            {!empIdResult &&
              filterResults.map((emp) => (
                ...
                >
                  <EmployeeCardWithoutCheckBox
                    employee={{
                      id: emp.payRollId || emp.tempPayrollId,
                      name: emp.empName,
