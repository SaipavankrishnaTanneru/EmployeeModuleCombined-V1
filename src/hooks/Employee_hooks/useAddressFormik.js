...
// import { addressFormSchema } from "../../utils/Employee_utils/OnboardingSchemas"; 
// import {
//   usePincodeQuery,
//   useCitiesByDistrict,
//   postAddressInfo,
//   useAddressGetQuery,
// } from "../../queries/Employee_queries/onBoardingForms/postApi/useAddressQueries";
...
const normalizeTextFields = (address) => ({
  ...address,
  addressLine1: toTitleCase(address.addressLine1),
  addressLine2: toTitleCase(address.addressLine2),
  addressLine3: toTitleCase(address.addressLine3),
});
...
export const useAddressFormik = ({ tempId, onSuccess }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      // ✅ Apply Title Case ONLY here
      const currentAddress = normalizeTextFields(values.currentAddress);
      const permanentAddress = isSame
        ? normalizeTextFields(values.currentAddress)
        : normalizeTextFields(values.permanentAddress);
      ...
      const payload = {
        ...
        currentAddress: {
          ...currentAddress,
          cityId: safeNum(currentAddress.cityId),
          districtId: safeNum(currentAddress.districtId),
          stateId: safeNum(currentAddress.stateId),
          countryId: safeNum(currentAddress.countryId) || 1,
        },

        permanentAddress: {
          ...permanentAddress,
          cityId: safeNum(permanentAddress.cityId),
          districtId: safeNum(permanentAddress.districtId),
          stateId: safeNum(permanentAddress.stateId),
          countryId: safeNum(permanentAddress.countryId) || 1,
        },
  ...
  useEffect(() => {
    if (!isDataPopulated && savedData) {
      ...
      if (savedData.currentAddress) {
        ...
        currData =
          Array.isArray(savedData.CURR) && savedData.CURR.length > 0
            ? savedData.CURR[0]
            : {};
        permData =
          Array.isArray(savedData.PERM) && savedData.PERM.length > 0
            ? savedData.PERM[0]
            : {};
        isSame = savedData.permanentAddressSameAsCurrent ?? false;
      }
  ...
  const { data: currPinData } = usePincodeQuery(values.currentAddress.pin);
  const { data: permPinData } = usePincodeQuery(
    !values.permanentAddressSame ? values.permanentAddress.pin : null
  );
  ...
  const { data: currCities = [] } = useCitiesByDistrict(
    values.currentAddress.districtId
  );
  const { data: permCities = [] } = useCitiesByDistrict(
  ...
  return {
    ...
    permStateName: permPinData?.stateName,
    permDistrictName: permPinData?.districtName,
    stateOptions: currPinData
      ? [{ id: currPinData.stateId, name: currPinData.stateName }]
      : [],
    districtOptions: currPinData
      ? [{ id: currPinData.districtId, name: currPinData.districtName }]
      : [],
    cityOptions: currCities,
    permStateOptions: values.permanentAddressSame
      ? currPinData
        ? [{ id: currPinData.stateId, name: currPinData.stateName }]
        : []
      : permPinData
      ? [{ id: permPinData.stateId, name: permPinData.stateName }]
      : [],
    permDistrictOptions: values.permanentAddressSame
      ? currPinData
        ? [{ id: currPinData.districtId, name: currPinData.districtName }]
        : []
      : permPinData
      ? [{ id: permPinData.districtId, name: permPinData.districtName }]
      : [],
    permCityOptions: values.permanentAddressSame ? currCities : permCities,
  };
};
